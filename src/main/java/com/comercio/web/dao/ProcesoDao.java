...
public class ProcesoDao {
	...
	public void limpiarRol(long id) {
		entityManager.createQuery("update Proceso SET rol_id=null where rol_id=:id").setParameter("id", id)
				.executeUpdate();
	...
	public void delete(long id) {
		entityManager.createQuery("update Proceso SET estado=0 where id=:id").setParameter("id", id).executeUpdate();
	}
	...
	public void Update(long id, String nombre, String descripcion, String enlace, String logo) {
		...
		entityManager.createNativeQuery(
				...
				.setParameter("logo", logo).setParameter("id", id).executeUpdate();

	}

	public void delRolProceso(long id) {
		entityManager.createNativeQuery("DELETE FROM procesos p WHERE ye.id IN (:list)").setParameter("id", id)
				.executeUpdate();
	}
	...
	public void UpdateRolProceso(Proceso procesos, long id) {
		entityManager.createNativeQuery("Update roles  Set procesos=:procesos Where id=:id")
				.setParameter("procesos", procesos).setParameter("id", id).executeUpdate();
	}
	...
	public List<Proceso> ProcesosAsignados(long id) {
		return entityManager.createNativeQuery(
				"select p from procesos p, roles r, roles_procesos rp where rp.rol_id = :id and p.id=rp.procesos_id and r.id=rp.rol_id",
				Proceso.class)
